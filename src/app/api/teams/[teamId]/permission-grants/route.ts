...
export async function POST(request: Request, { params }: { params: Promise<{ teamId: string }> }) {
  ...
  // Scoped to the group the new grant applies to: a team-wide GRANT_PERMISSIONS
  // holder can grant anything, and someone with a GRANT_PERMISSIONS grant scoped
  // to this exact group can grant other group-scoped permissions within it — but
  // never a team-wide grant, and never a grant scoped to a different group.
  if (!(await hasTeamPermission(user.id, teamId, "GRANT_PERMISSIONS", positionGroupId))) {
    return NextResponse.json({ error: "Not authorized." }, { status: 403 });
  }
...
export async function DELETE(request: Request, { params }: { params: Promise<{ teamId: string }> }) {
  ...
  // Scoped to the grant being revoked, same as granting: a team-wide
  // GRANT_PERMISSIONS holder can revoke anything, a group-scoped holder only
  // grants within that same group.
  if (!(await hasTeamPermission(user.id, teamId, "GRANT_PERMISSIONS", grant.positionGroupId))) {
    return NextResponse.json({ error: "Not authorized." }, { status: 403 });
  }
