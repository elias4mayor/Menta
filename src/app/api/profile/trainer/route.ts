...
export async function PATCH(request: Request) {
  ...
  if (user.role !== "TRAINER") {
    return NextResponse.json({ error: "This endpoint is for trainer accounts." }, { status: 403 });
  }
  ...
  await logAudit({ actorId: user.id, action: "profile.updated", targetType: "TrainerProfile", targetId: profile.id });
